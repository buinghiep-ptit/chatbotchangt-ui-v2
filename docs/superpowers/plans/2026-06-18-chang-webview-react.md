...
export function TasksPanel() {
  // NOTE: do NOT subscribe via `useWidgetStore((s) => s.filteredTasks())` —
  // that store method returns a fresh array each call, so Zustand's snapshot
  // changes every render and triggers an infinite update loop ("Maximum update
  // depth exceeded"). Select stable refs and derive in render instead.
